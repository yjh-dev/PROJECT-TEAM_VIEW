...
워커에 대해:
  ...
  1. **폴러**(`tools/worker.ps1`, 권장): 대화형 세션이 아니라 큐를 지켜보다가
     `claude -p --continue`로 지시를 하나씩 실행하는 루프다. 클레임에 `mode: "poller"`를
     적고 **스스로 30초마다 갱신한다.** 폴러가 띄운 claude에는 `TEAMVIEW_POLLER`가
     환경변수로 실려 오므로 훅은 그 세션을 알아본다.
  2. **대화형 워커**(`TEAMVIEW_WORKER=1`, 예전 방식): 세션이 자기 session_id를 적는다.

  왜 폴러로 바꿨나: 대화형 워커의 클레임과 대기열 처리는 둘 다 **훅이 돌 때만** 일어난다.
  그런데 `Stop` 훅은 세션이 한 턴을 끝낼 때만 돈다 — 갓 띄운 워커는 입력을 받은 적이 없어
  턴이 없고, 그래서 큐가 영원히 쌓였다. 게다가 유휴 워커는 훅이 안 도니 클레임이 10분 뒤
  만료돼, 살아 있는데도 분리가 풀려 **사람과 대화하는 세션이 큐를 가져갔다.**

  **살아 있는 폴러 클레임이 최우선이다.** 그 동안 훅은 큐를 건드리지 않는다(같은 지시가
  두 번 실행되면 안 된다). 클레임이 없거나 오래되면 예전처럼 아무 세션이나 처리한다 —
  워커가 없다고 앱이 먹통이 되면 안 된다.
"""
import json
import os
import re
