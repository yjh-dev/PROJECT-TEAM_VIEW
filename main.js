  ...
  const task =
    who && who !== 'lead'
      ...
      : `다음 지시를 읽고 성격에 맞는 서브에이전트에게 위임해서 처리해줘.` +
        ` 여러 파트가 걸리면 planner로 나눈 뒤 각자에게 넘겨줘.` +
        ...
        // **비싼 것은 위임 자체가 아니라 위임하는 방식이다.** 위임을 줄이면 품질이 깎이지만,
        // 아래 두 가지는 같은 일을 그대로 하면서 비용만 뺀다.
        //
        // 실측(insta-filter): 리드가 `Agent` 20회, `SendMessage` 25회를 불렀다.
        //   · `Agent`      = 팀원을 **새로 띄운다** → 그 팀원이 프로젝트를 처음부터 다시 읽는다
        //   · `SendMessage`= 이미 띄운 팀원에게 **이어서 시킨다** → 그 팀원의 컨텍스트를 재사용한다
        // 같은 팀원을 일마다 새로 띄우면 그때마다 같은 파일을 다시 읽는다. 그 비용은
        // 이 세션 기록에 **남지도 않는다**(팀원 턴은 isSidechain으로 따로 돈다) — 보이지
        // 않을 뿐 계정에는 그대로 청구된다.
        `\n\n[위임은 하되, 두 번 일하게 하지 마라]` +
        ` ① **이미 부른 팀원에게는 이어서 시켜라.** 같은 팀원이 또 필요하면 새로 띄우지 말고` +
        ` 그 팀원에게 이어 말해라 — 새로 띄우면 프로젝트를 처음부터 다시 읽는다.` +
        ` ② **네가 이미 알아낸 것은 넘겨줘라.** 파일 경로, 원인, 정한 방식처럼 네가 확인한 것을` +
        ` 위임하는 말에 함께 적어라. 안 적으면 팀원이 같은 것을 다시 찾느라 처음부터 훑는다.` +
        ` 맡기는 범위도 좁게 적어라 — "고쳐줘"보다 "이 파일의 이 함수를 이렇게"가 싸고 정확하다.` +
        ` 지시: ${body}`
...
function runCommand(c, cmd) {
  ...
  const finishExit = (code) => {
    ...
    if (!pendingCount(dir)) {
      // **끝까지 돈 것을 실패라고 하지 마라.** `failureFor`는 세션 기록에서 `is_error`가
      // 붙은 줄을 찾는데, 그건 **도중에 실패한 도구 호출 하나**일 뿐이다 — 팀원이
      // `npm test`를 돌려 보고 실패를 확인하는 것은 일의 정상적인 일부다.
      //
      // 실측(08-14 00:57): qa-tester가 테스트를 여러 번 돌린 뒤 리드가 "README에 두 줄
      // 추가하고 검수까지 마쳤습니다"라고 답했고 파일에도 반영됐다. 1초 뒤 앱은
      // **"지시 실패"**를 띄웠다. 성공한 일을 실패로 보고한 것이고, 이 앱이 가장
      // 경계하는 종류다(없는 것을 있다고 말하는 것의 반대 방향).
      //
      // 종료 코드가 0이면 그 실행은 끝까지 돌았다. 중간의 도구 실패는 사유가 아니다.
      const why = code === 0 ? null : failureFor(dir, sid, startedAt, code, { stdout: outTail.text(), stderr: errLines.join('\n') })
      notifyDone(dir, why)
    }
  }
...
ipcMain.handle('command:send', async (_e, { dir, agent, text, broadcast }) => {
  ...
  // **무게는 지시를 보내는 순간에 알려야 한다.** 사용량 화면에도 적어 두지만 그건
  // 열어 봐야 보인다 — 그래서 대화가 무거워져도 아무도 갈아타지 않는다. 실측:
  // insta-filter는 응답 215개까지 자라 질문 하나에 183k를 다시 읽고 있었고, 갈아탄 뒤
  // 41k가 됐다(4.4분의 1). 그동안 화면에는 계속 적혀 있었지만 아무도 보지 않았다.
  return {
    ok: true,
    dir: projectDir,
    companyOpen: Boolean(c),
    busy,
    hold: holdInfo(projectDir),
    session: sessionCost(projectDir),
  }
})
