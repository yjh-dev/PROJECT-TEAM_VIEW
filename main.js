...
function promptFor(cmd, dir) {
  ...
  const task =
    who && who !== 'lead'
      ...
      : `다음 지시를 읽고 처리해줘.` +
        // **팀원을 부르는 것이 이 앱에서 가장 비싼 동작이다.** 부른 팀원은 자기 컨텍스트를
        // 새로 쌓고, 그것이 전부 턴이 되어 대화에 눌러앉는다. 한 번 들어온 것은 그 세션이
        // 끝날 때까지 매 호출에 다시 실린다. 실측(insta-filter 258턴 / 지시 34개):
        //   · 지시 하나가 평균 **7.6턴**을 만든다
        //   · 캐시 생성 5.57M 중 **71%(3.97M)가 첫 턴이 아닌 뒤따르는 턴**에서 나왔다
        //     — 즉 위임이 만들어 낸 턴들이다
        //   · 캐시 생성은 읽기보다 12.5배 비싸다. 가중 환산하면 전체 비용의 69%다
        // 그래서 기본값을 뒤집는다. 위임은 **나눌 이유가 있을 때** 하는 것이지 기본이 아니다.
        ` **먼저 이 일이 팀을 부를 만한 일인지 판단해라.**` +
        ` 파일 한두 개를 고치는 일, 명백한 버그 수정, 이미 아는 것에 답하는 일은 **네가 직접 해라** —` +
        ` 팀원을 부르면 그 팀원이 처음부터 다시 파악해야 해서 같은 일을 두 번 하게 된다.` +
        ` 여러 파트가 걸리거나(화면+서버), 설계 판단이 필요하거나, 코드를 새로 쓰는 분량이면` +
        ` 그때 **성격에 맞는 팀원에게 Task로 맡겨라.** 여러 파트면 planner로 나눈 뒤 각자에게 넘겨라.` +
        // "직접 하지 말고 위임하라"가 **묻는 말에까지** 적용돼 질문 하나에 팀이 통째로
        // 동원됐다. 조사는 scout 한 명이면 되고, 아는 것이면 그냥 답하면 된다.
        ` **묻는 말**(현황·상태·구조·이유)이라면 팀을 부르지 마라 —` +
        ` 이미 아는 것이면 바로 답하고, 확인이 필요하면 조사역(scout) 한 명에게만 맡겨라.` +
        // 위임을 줄이라는 말이 **검수를 건너뛰라**는 말로 읽히면 안 된다. 예전에 BOUNDARY의
        // "요청하지 않은 산출물을 만들지 마라"가 그렇게 읽혀 116분 작업에 QA가 0번 불렸다.
        ` 다만 **네가 직접 고쳤더라도 검수(qa-tester)는 건너뛰지 마라** — 그건 위임이 아니라 확인이다.` +
        ` 지시: ${body}`
